    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        switch ($args[0]) {
            ...
            case "create":
                ...
                if(isset($this->plugin->arenaonevsone[$args[1]])) {
                    $sender->sendMessage("§c> Arena $args[1] already exists!");
                    break;
                }
                $this->plugin->arenaonevsone[$args[1]] = new OneVsOne($this->plugin, []);
            ...
            case "remove":
                ...
                $arena = $this->plugin->arenaonevsone[$args[1]];

                foreach ($arena->players as $player) {
                    $player->teleport($this->plugin->getServer()->getDefaultLevel()->getSpawnLocation());
                }

                if(is_file($file = $this->plugin->getDataFolder() . "arenaonevsone" . DIRECTORY_SEPARATOR . $args[1] . ".yml")) unlink($file);
                unset($this->plugin->arenaonevsone[$args[1]]);
            ...
            case "set":
                ...
                if(isset($this->plugin->setteronevsone[$sender->getName()])) {
                ...
                $this->plugin->setteronevsone[$sender->getName()] = $this->plugin->arenaonevsone[$args[1]];
                break;
            case "arenas":
                ...
                if(count($this->plugin->arenaonevsone) === 0) {
                    $sender->sendMessage("§6> There are 0 arenas.");
                    break;
                }
                $list = "§7> Arenas:\n";
                foreach ($this->plugin->arenaonevsone as $name => $arena) {
                    if($arena->setup) {
